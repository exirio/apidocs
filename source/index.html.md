...
toc_footers:
  - <a href='https://exir.io/account/developers'>Sign Up for a Developer Key</a>
...
EXIR provides a complete RESTful API for developers which allows full access to all the functionalities on the exchange. EXIR also support websocket for real-time updates.
...
EXIR uses API tokens to allow private access to the API for an authenticated user. You can register a new EXIR API key in our [developer portal](https://exir.io/developers).
...
```shell
curl -X GET "https://api.exir.io/v0/ticker?symbol=btc-eur"
```
...
`GET https://api.exir.io/v0/ticker`
...
```shell
curl -X GET "https://api.exir.io/v0/orderbooks?symbol=btc-eur"
```
...
`GET https://api.exir.io/v0/orderbooks`
...
```shell
curl -X GET "https://api.exir.io/v0/trades?symbol=btc-eur"
```
...
`GET https://api.exir.io/v0/trades`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN"
  "https://api.exir.io/v0/user/balance" 
```
...
`GET https://api.exir.io/v0/user/balance`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN" 
  "https://api.exir.io/v0/user/deposits
...
`GET https://api.exir.io/v0/user/deposits`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN" 
  "https://api.exir.io/v0/user/withdrawals
  ?currency=btc&limit=50&page=1&order=asc"
```
...
`GET https://api.exir.io/v0/user/withdrawals`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN"
  "https://api.exir.io/v0/user/withdraw/$currency/fee"
```
...
`GET https://api.exir.io/v0/user/withdraw/{currency}/fee`
...
curl -X POST -H "Authorization: Bearer $ACCESS_TOKEN"
	...
	"https://api.exir.io/v0/user/request-withdrawal"
```
...
`POST https://api.exir.io/v0/user/request-withdrawal`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN" 
  "https://api.exir.io/v0/user/trades
...
`GET https://api.exir.io/v0/user/trades`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN"
  "https://api.exir.io/v0/user/orders?symbol=$symbol" 
```
...
`GET https://api.exir.io/v0/user/orders`
...
```shell
curl -X GET -H "Authorization: Bearer $ACCESS_TOKEN"
  "https://api.exir.io/v0/user/orders/$orderId" 
```
...
`GET https://api.exir.io/v0/user/orders/{orderId}`
...
`POST https://api.exir.io/v0/order`
...
`DELETE https://api.exir.io/v0/user/orders`
...
`DELETE https://api.exir.io/v0/user/orders/{orderId}`
...
```javascript
  ...
  const socket = io('https://api.exir.io/user', {
...
Parameter | Description
--------- | -----------
query | You must provide your EXIR API token (`token`)
